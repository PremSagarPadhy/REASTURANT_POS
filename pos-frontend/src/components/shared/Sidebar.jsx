...
const Sidebar = () => {
  ...
  const handleMenuItemClick = (item) => {
    if (item.label === "Analytics" || item.label === "Edit Orders" || item.label === "View Orders"|| item.label === "Create Orders" || item.label === "Payments" 
      ...
      || item.label === "Invoices" || item.label === "Inventory" || item.label === "Customer" || item.label === "Employee Details" ||item.label==="Employee Edit"|| item.label === "Employee Attendance" || item.label === "Employee Working" || item.label === "Feedback" 
  ...
  const menuItems = [
    ...
    { icon: <GrUserWorker size={20} />,
        ...
        { label: "Employee Working", action: () => handleMenuItemClick({ label: "Employee Working" }) }
      ], 
    },
